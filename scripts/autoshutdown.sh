...
if screen -list | grep -q "mcs"; then
        ...
        PLAYERSLIST=$(sudo perl -ne '$l=$_ if /There are/; END{print $l}' /home/minecraft/logs/latest.log | cut -f2
 -d"/" | cut -f2 -d ":")
        ...
        then
                ...
                $(screen -S mcs -p 0 -X stuff "list^M")
                sleep 5
                PLAYERSLIST=$(sudo perl -ne '$l=$_ if /There are/; END{print $l}' /home/minecraft/logs/latest.log |
 cut -f2 -d"/" | cut -f2 -d ":")
